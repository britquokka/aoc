...
class Puzzle:
    ...
    def __init__(self, file):
        program_memory = Puzzle.to_program_memory(file)
        self.scanner = MemoryScanner(program_memory)

    def run_program(self):
        instructions = self.scanner.get_mul_instructions()
        logger.debug(instructions)
        total = sum([op1 * op2 for offset, (name_instruction, (op1, op2)) in instructions])
        return total

    def run_program_part2(self):
        instructions = []
        instructions.extend(self.scanner.get_mul_instructions())
        instructions.extend(self.scanner.get_other_instructions("do()"))
        instructions.extend(self.scanner.get_other_instructions("don't()"))
        instructions.sort()
        total = 0
        enable = True
        for idx, instruction in instructions:
            if instruction.name == "mul":
                if enable:
                    ...
                    total += op1 * op2
            elif instruction.name == "don't()":
                enable = False
            elif instruction.name == "do()":
                enable = True
        return total
