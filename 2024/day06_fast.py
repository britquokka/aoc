...
class PathFinder:
    ...
    def find_exit_path(self, initial_state, already_visited=None):
        flag_exit_loop = False
        has_loop = False
        state = initial_state
        full_path = []
        visited = set(already_visited) if already_visited else set()
        while not flag_exit_loop:
            ...
            if not has_exited and next_state in visited:
                has_loop = True
            flag_exit_loop = has_loop or has_exited
            state = next_state
        return full_path, has_loop
...
class Puzzle:
    ...
    def count_obstruction_candidates(self):
        ...
        path, _ = self.path_finder.find_exit_path(initial_guard_state)
        idx = -1
        already_tested = set(start)
        for state, next_state in zip(path, path[1:]):
            idx += 1
            (y, x), _ = next_state
            if (y, x) not in already_tested:
                prev = self.area.grid[y][x]
                self.area.grid[y][x] = '#'
                _, has_loop = self.path_finder.find_exit_path(state, path[0:idx])
                if has_loop:
                    count += 1
                self.area.grid[y][x] = prev
                already_tested.add((y, x))
        return count
